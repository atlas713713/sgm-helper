...
| --- | --- | --- |
| 米饭 | `Tiramisu64_20` | `127.0.0.1:5755` |
| 地球 | `Tiramisu64_21` | `127.0.0.1:5765` |
| 地球瘦子 | `Tiramisu64_22` | `127.0.0.1:5775` |
| 栗威 | `Tiramisu64_23` | `127.0.0.1:5785` |

项目 Debug 专用模拟器为 `127.0.0.1:5745`。手工调试安装和设备测试只使用这个目标；上面的四台是正式部署目标。正式安装时每台只尝试一次；ADB 连接或 `install -r` 失败就记录并跳过，不重试、不为此重启模拟器。
...
只修改文档时不需要重新构建 APK。Debug 更新只安装到 `127.0.0.1:5745`；正式发布时才安装到四台部署目标。
...
```sh
./restart_bluestacks_instance.sh Tiramisu64_21 127.0.0.1:5765
```
...
- 每次 APK 更新必须递增应用版本，并默认向四台正式模拟器各尝试安装一次；失败目标记录后跳过，不重试。
